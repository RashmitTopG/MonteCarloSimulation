...
def update_graphs(num_simulations):
    ...
    fig_simulation = px.histogram(revenues, nbins=100, title="Revenue Distribution").update_layout(
        ...
        title_x=0.5, font=dict(family="Arial", size=14, color="#ffffff")
    )
    ...
    fig_box_plot = px.box(x=revenues, title="Box Plot of Revenues").update_layout(
        xaxis_title='Revenue', template=template,
        title_x=0.5, font=dict(family="Arial", size=14, color="#ffffff")
    )

    # CDF plot
    fig_cdf = px.ecdf(x=revenues, title="Cumulative Distribution Function (CDF) of Revenues").update_layout(
    ...
    risk_values = [np.mean(lead_time_risks), np.mean(defect_rate_risks), np.mean(shipping_cost_risks), np.mean(manufacturing_cost_risks)]
    
    fig_bar_chart = px.bar(x=risk_factors, y=risk_values, title="Average Values of Risk Factors").update_layout(
        xaxis_title='Risk Factors', yaxis_title='Average Value', template=template,
        title_x=0.5, font=dict(family="Arial", size=14, color="#ffffff")
    )

    # Defect rate histogram
    fig_defect_rate_hist = px.histogram(defect_rate_risks, nbins=50, title="Defect Rate Histogram").update_layout(
        xaxis_title='Defect Rate', yaxis_title='Frequency', template=template,
        title_x=0.5, font=dict(family="Arial", size=14, color="#ffffff")
    )

    # Pie chart for major risk factors
    fig_pie_chart = px.pie(names=risk_factors, values=risk_values, title="Proportion of Major Risk Factors").update_layout(
        title_x=0.5, template=template,
        font=dict(family="Arial", size=14, color="#ffffff")
    )

    # Major risk factor bar chart
    fig_major_risk_factor = px.bar(x=risk_factors, y=risk_values, title="Major Risk Factors").update_layout(
        xaxis_title='Risk Factors', yaxis_title='Values', template=template,
        title_x=0.5, font=dict(family="Arial", size=14, color="#ffffff")
    )

    simulation_info = f"Mean Revenue: ${stats['mean_revenue']:.2f}, Std Dev: ${stats['std_revenue']:.2f}, 5th Percentile: ${stats['percentile_5']:.2f}, 95th Percentile: ${stats['percentile_95']:.2f}"
